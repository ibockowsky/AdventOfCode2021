...
fileInterface.on('close', () => {
    ...
    for (let i = 0; i < bingoNumbers.length; i++) {
        if (i > 4) {
            const bingos = new Array()
            for (let j = 0; j < bingoGrids.length; j++) {
                const isBingo = checkRows(bingoGrids[j]) || checkCols(bingoGrids[j])

                if (isBingo) {
                    bingoBoard = bingoGrids[j]
                    bingos.push(bingoGrids[j])
                }
            }

            bingos.forEach(_bingo => {
                bingoGrids = bingoGrids.filter(el => JSON.stringify(el) !== JSON.stringify(_bingo))
            })

            if (!bingoGrids.length) break
        }
